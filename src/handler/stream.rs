...
pub fn save_file(field: Field) -> impl Future<Item=UploadResponse, Error=Error> {
    ...
    let params = match field.content_disposition() {
        ...
        None => return Either::A(err(error::ErrorBadRequest("Form data key or content is empty")))
    };
    ...
    let file_type = match vec.pop() {
        Some(typ) => {
            if typ != "jpg" && typ != "png" && typ != "gif" {
                return Either::A(err(error::ErrorBadRequest(format!(".{} can't be uploaded", typ))));
            }
            typ
        }
    ...
    Either::B(field
        .fold((UploadResponse::new(origin_filename, new_filename), file),
