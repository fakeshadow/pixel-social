...
use actix::{ActorFuture, AsyncContext, Context, ContextFutureSpawner, fut::Either as ActorEither, WrapFuture};
use futures::{
...
use futures01::{
    ...
    Future as Future01, IntoFuture, Stream,
};
use psn_api_rs::{PSN, PSNRequest as PSNRequestLib};
use redis::aio::SharedConnection;

use crate::handler::{
    cache::{CacheService, CheckCacheConn, GetQueue, GetSharedConn},
    db::{DatabaseService, GetDbClient, Query, SimpleQuery},
};
use crate::handler::cache::AddToQueue;
use crate::model::{
    actors::PSNService,
    errors::ResError,
    psn::{
...
impl CheckCacheConn for PSNService {
    ...
    fn replace_cache(&self, c: SharedConnection) {
        self.cache.as_ref().map(|cell| cell.replace(c));
    }
}
...
impl DatabaseService {
    ...
    pub fn get_trophy_titles(
        ...
        np_id: &str,
        page: u32,
    ) -> impl Future<Output=Result<Vec<UserTrophyTitle>, ResError>>
    {
        let query = format!(
        ...
        self.simple_query_multi_trait::<UserTrophyTitle>(query.as_str(), Vec::with_capacity(20))
            .and_then(|v| {
                if v.is_empty() {
                    futures::future::err(ResError::NotFound)
                } else {
                    futures::future::ok(v)
                }
            })
...
impl CacheService {
    ...
    pub fn add_psn_request_privilege(
        &self,
        key: &str,
    ) -> impl Future<Output=Result<(), ResError>>
    {
        self.add_to_queue_01(key, 0).compat()
    }

    pub fn add_psn_request_now(
        &self,
        key: &str,
    ) -> impl Future<Output=Result<(), ResError>>
    {
        self.add_to_queue_01(key, chrono::Utc::now().timestamp_millis()).compat()
    }
}
