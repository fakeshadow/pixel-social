            ...
            <template v-slot:activator="{ on }">
                <v-btn icon large v-on="on">
                    <v-avatar size="32px" tile>
                        <img v-if="!psn_profile.avatarUrl" src="../assets/playstation-brands.svg">
                        <img v-if="psn_profile.avatarUrl" v-bind:src="psn_profile.avatarUrl">
                    </v-avatar>
                </v-btn>
            </template>
            ...
            <v-card>
                <v-list>
                    <v-list-tile avatar>
                        ...
                        <v-list-tile-content>
                            <v-list-tile-title>{{profile.id}}</v-list-tile-title>
                ...
                <v-divider></v-divider>
                <v-list>
                    <v-list-tile @click="openLinkPSNDialog" v-if="profile.id =='not linked'">
                        <v-dialog v-model="linkPSNDialog" max-width="300">
                            <v-card>
                                <v-layout row wrap justify-center text-xs-center>
                                    <v-flex xs12>
                                        <v-card-text text-xs-center>
                                            <v-text-field v-model="linkingPSNId" label="Your PSN ID"
                                                          solo></v-text-field>
                                            <h3
                                                    class="red--text"
                                            >Please change your "About Me" comment in the PSN account settings to the
                                                exact match of the following code before continue:</h3>
                                            <v-text-field
                    ...
                    <v-list-tile to="/mypage" v-if="profile.id !=='not linked'">
                        <v-list-tile-avatar>
                            <v-icon>account_box</v-icon>
                        </v-list-tile-avatar>
                        <v-list-tile-title>My Page</v-list-tile-title>
                    </v-list-tile>
                    <v-list-tile @click="logout" v-if="profile.id !=='not linked'">
                        <v-list-tile-avatar>
        ...
        methods: {
            ...
            async linkingPSN() {
                try {
                    ...
                    localStorage.psn_profile = JSON.stringify(psnProfile);
